...
class Backend:
    ...
    def ba(self, t_start, t_end, steps, graph, nms, radius, thresh, max_factors, t_start_loop=None, loop=False, motion_only=False):
                ...
                for si in range(max(i-n_neighboring, t_start_loop), min(i+n_neighboring+1, t_end)):
                    for sj in range(max(j-n_neighboring, t_start), min(j+n_neighboring+1, t_end)):
                        if rawd[(si-t_start_loop), (sj-t_start)] <= thresh:
                            num_loop += 1
                            if si != sj and si > t_start and sj > t_start:
                                sub_es += [(si, sj)]
        ...
        graph.update_lowmem(
            t0=t_start_loop,  # fix the start point to avoid drift, be sure to use t_start_loop rather than t_start here.
            t1=t_end,
            iters=2,
            use_inactive=False,
            steps=steps,
    ...
    @torch.no_grad()
    def dense_ba(self, t_start, t_end, steps=1, motion_only=False):

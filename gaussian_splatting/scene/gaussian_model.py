...
from utils.mask_utils import get_depth_mask
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree
        self.max_depth = 100.0
        self.max_depth_sigma_thresh = 0.2
    ...
    def create_pcd_from_image(self, cam_info,pyr_level,pyr_scaling, init=False, scale=2.0, depthmap=None):
        cam = cam_info
        image_ab = (torch.exp(cam.exposure_a)) * cam.original_image + cam.exposure_b
        image_ab = torch.clamp(image_ab, 0.0, 1.0)
        rgb_raw = (image_ab * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy()
        depthmap = depthmap.contiguous().cpu().numpy()
        cov = cam_info.cov.contiguous().cpu().numpy()
        mask = get_depth_mask(cov,self.max_depth_sigma_thresh)

        depthmap = np.where(~mask, depthmap, self.max_depth + 1.0 )
        ...
        return self.create_pcd_from_image_and_depth(cam, rgb, depth,pyr_level,pyr_scaling, init)

    def create_pcd_from_image_and_depth(self, cam, rgb, depth, pyr_level,pyr_scaling, init=False):
        if init:
            downsample_factor = self.config["Dataset"]["pcd_downsample_init"] / pow(1 / pyr_scaling, pyr_level * pyr_level)
        else:
            downsample_factor = self.config["Dataset"]["pcd_downsample"] / pow(1 / pyr_scaling, pyr_level * pyr_level)
        point_size = self.config["Dataset"]["point_size"] * pow(1 / pyr_scaling,pyr_level)
    ...
    def extend_from_pcd_seq(
        ...
        fused_point_cloud, features, scales, rots, opacities = (
            self.create_pcd_from_image(cam_info,pyr_level,pyr_scaling, init, scale=scale, depthmap=depthmap)
        )

...
class SLAM_GUI:
    ...
    def init_widget(self):
        ...
        self.in_cov_widget = gui.ImageWidget()
        tab_info.add_child(gui.Label("Input Color/Depth/Cov"))
        tab_info.add_child(self.in_rgb_widget)
        tab_info.add_child(self.in_depth_widget)
        tab_info.add_child(self.in_cov_widget)
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.gtdepth is not None:
            ...
            rgb = o3d.geometry.Image(output)
            self.in_depth_widget.update_image(rgb)

        

        if gaussian_packet.cov is not None:

            sigma = np.sqrt(gaussian_packet.cov)
            sigma = torch.from_numpy(sigma).squeeze().to(torch.float)
            valid = (sigma > 0) # one shouldn't use exact equality on floats but for synthetic values it's ok
            dmin = sigma.min().item()  
            dmax = sigma.max().item()

            output = (sigma - dmin) / (dmax - dmin)
            output[output < 0] = 0 # saturate
            output[output > 1] = 1 # saturate
            #output = output / (output + 0.1)# 0 -> white, 1 -> black
            output[~valid] = 1 # black out invalid
            ...
            rgb = o3d.geometry.Image(output)
            self.in_cov_widget.update_image(rgb)
...
def run(params_gui=None):
    print('Splat Visualization Triggered!')
    app = o3d.visualization.gui.Application.instance
    app.initialize()
